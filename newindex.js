...
const weather = require('weather-js');
  ...
  app.get('/jakarta', function (req, res){
    weather.find({search: 'Jakarta, Indonesia', degreeType: 'C'}, function(err, result) {
        if(err) console.log(err);
        else{
            let data={
                weatherdavao: eval(JSON.stringify(result,null,2))
            }
            res.render('jakarta',data)
        }
      });
  })

  app.get('/berlin', function (req, res){
    weather.find({search: 'Berlin, Germany', degreeType: 'C'}, function(err, result) {
        if(err) console.log(err);
        else{
            let data={
                weatherdavao: eval(JSON.stringify(result,null,2))
            }
            res.render('berlin',data)
        }
      });
  })

  app.get('/ottawa', function (req, res){
    weather.find({search: 'Ottawa, Canada', degreeType: 'C'}, function(err, result) {
        if(err) console.log(err);
        else{
            let data={
                weatherdavao: eval(JSON.stringify(result,null,2))
            }
            res.render('ottawa',data)
        }
      });
  })

  app.get('/moscow', function (req, res){
    weather.find({search: 'Moscow, Russia', degreeType: 'C'}, function(err, result) {
        if(err) console.log(err);
        else{
            let data={
                weatherdavao: eval(JSON.stringify(result,null,2))
            }
            res.render('moscow',data)
        }
      });
  })
